...
# 1. Remove the PropertyGroup and the ImportGroup that added by us.
# 2. Remove the .wpp.targets file if it is empty.
...
function CleanupWppTargetsFile($project, $toolsPath) {    
    $projName = $project.Name
    $projDirectory = $project.Properties.Item('FullPath').Value
    
	$wppTargetsName = "{0}.wpp.targets" -f $projName
    $wppTargetsPath = Join-Path $projDirectory -ChildPath $wppTargetsName
    ...
    if(($wppTargetsExists)) {
		...
		RemovePropertyGroups -projectRootElement $projRoot
		RemoveImportGroups -projectRootElement $projRoot
		Write-Debug ("    Child element count: {0}" -f $projRoot.Count) | Out-Null
		$projectIsEmpty = ($projRoot.Count -eq 0)   
		$projCollection.UnloadProject($projRoot)
		if ($projectIsEmpty) {
			$wppTargetsFile = $project.ProjectItems | Where-Object{$_.Name -eq $wppTargetsName}
			if (!($wppTargetsFile -eq $null)) {
				$wppTargetsFile.Delete()
			}
		}
	}
}
...
function RemovePropertyGroups($projectRootElement){
    ...
    foreach($pg in $projectRootElement.PropertyGroups){
        if($pg.Label -and [string]::Compare($pwMsbuildLabel,$pg.Label,$true) -eq 0) {
    ...
    foreach($pg in $pgsToRemove){
		Write-Debug ("    Removing PropertyGroup {0}" -f $pg.Label) | Out-Null
...
function RemoveImportGroups($projectRootElement){
    $igToRemove = @()
    foreach($ig in $projectRootElement.ImportGroups){
        if($ig.Label -and [string]::Compare($pwMsbuildLabel,$ig.Label,$true) -eq 0) {
            # remove this import
            $igToRemove += $ig
        }
    }

    foreach($ig in $igToRemove){
		Write-Debug ("    Removing ImportGroup {0}" -f $ig.Label) | Out-Null
...
CleanupWppTargetsFile $project $toolsPath
# SIG # Begin signature block
# MIIgSgYJKoZIhvcNAQcCoIIgOzCCIDcCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUVVJh8QZ/876nUwMSiE5jDWwr
# EuqgghsfMIIEwzCCA6ugAwIBAgITMwAAAHeHqQZ/e88wOQAAAAAAdzANBgkqhkiG
...
# KwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFICe6YG5
# yl962yOcJ5gLEP1h3HVOME4GCisGAQQBgjcCAQwxQDA+oCSAIgBNAGkAYwByAG8A
# cwBvAGYAdAAgAEEAUwBQAC4ATgBFAFShFoAUaHR0cDovL3d3dy5hc3AubmV0LyAw
# DQYJKoZIhvcNAQEBBQAEggEAXXb+9DWwfH/72oW/Zw2fu1v4hFoVNqno8xcee1i5
# Y+E/RDt6d0ZmLsBTWu+Qs+FIzvRKvalEfUr30qHsFOb8FnAWhbrc72FkkUXKSo55
# QUjmDCWiSWa3vlVDW3LjYxTICvkeCH82xVnLi4DbyXFDjAI3ynMOundaXhDCmPgq
# VaZBaLBe3QJJEU5dJ92Ut+ZoVJBtHtCL58d27yYxHKyK4X2dfkQGRtUbBgcz/u/3
# BXg13YsQd4nYeu0n0r1YDemQKAe1VdRq/CfgPWykheBOeU37MqApTUBb0kyudz3M
# PGYXQPFEoF0ZnhZalZHIItORZeb8lr3RQTYksYJ7mawpgqGCAigwggIkBgkqhkiG
...
# AQkEMRYEFAaRmw16MfvdESXHxc4JjwGTP5dPMA0GCSqGSIb3DQEBBQUABIIBAK2S
# jBG7PZUyW5n8ghEOmEMj/bgwu2sn3mVduAini86TdV9npv63IfXxKPHc2gH8NB3B
# aDFDc0HZXqJxUBN43WRPoskUKRk53fFqzGc19Bu8z2Z+ttRfQoxNF4YiRLN75P1p
# RGKKe4WvJstVOQjoRZGYbdmoUqmv6ZteylM0nV7KW4Hk5LgDodQ6+wblkmgGa9CD
# QEtxA/uN1hLnn5AqNsNqf9UXezspZggV2hEy6A/Ib3xp/EgPKU9M3lxKIH2Aa/ds
# 4aHM08IU7eDHIXyDNWZntMqLRxo5lEAdCbk8ov2lv9MgpdAzW1iT7LJLqKnY6K6c
# dofkLF7yG300477h4uQ=
